...
###### Data Selection: 
Because geographic data is limited by Twitter's privacy policies, I chose to evaluate mentions: @sebastianpinera, which is the Twitter account of Chile's President Sebastian Pinera.
...
The python and R codes are below.
        ...
        library(wordcloud)
        library(data.table)
        library(dplyr) ##check to see if this is in the tidyverse

        ##Create Twitter API token

        token <- rtweet::create_token(
          app = "Advanced Data Programming",
          consumer_key <- "vJ9Fq7nc7njx1BAODRPTJN3rP",
          consumer_secret <- "MpFXe39qsFAic2FlraQ6b43xjm3BPFRJ7pIAXQs5GSc85VZGhS",
          access_token <- "2462497903-szkV74Ro1BLdhJWnlYh5iBBOQZgBoHjWWA3kS9X",
          access_secret <- "KbuUezdN4aW2P1GjU8TmvReSz34gFIpxVydfquMWX4XZP")
